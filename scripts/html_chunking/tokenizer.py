...
try:
    ...
    raise ImportError(
        "LlamaIndex is required for token counting. "
...
try:
    ...
    raise ImportError(
        "HuggingFace transformers is required for token counting. "
...
class TokenCounter:
    ...
    def count_tokens(self, text: str) -> int:
        ...
        try:
            ...
            sequence_too_long = "sequence length is longer than the specified maximum"
            if sequence_too_long in str(e) and self.hf_tokenizer is not None:
                warnings.warn(
                    f"Token counting using full text failed: {e}. "
                    "Using manual chunking approach."
                )
                words = re.findall(r'\b\w+\b|[^\w\s]', text)
                chunks = [
                    ' '.join(words[i:i+WORDS_PER_BATCH])
