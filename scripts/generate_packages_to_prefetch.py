...
"""Legacy Cachito-era helper (obsolete).

Konflux/Cachi2 lockfiles are generated by ``scripts/konflux_requirements.sh`` and
``make update-konflux-deps``. Do not use this script.
...
if __name__ == "__main__":
    print(
        ...
        "  make update-konflux-deps          # CPU hashed lockfiles + .tekton binary.packages\n"
        "See scripts/konflux_requirements.sh and the README.",
        file=sys.stderr,
    )
    raise SystemExit(2)
