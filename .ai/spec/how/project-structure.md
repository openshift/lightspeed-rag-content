...
| Path | Purpose |
|---|---|
| `document_processor.py` | `DocumentProcessor` class -- orchestrates document loading, chunking, embedding, and vector store persistence. Delegates to `_LlamaIndexDB` (FAISS/PostgreSQL) or `_LlamaStackDB` (llama-stack faiss/sqlite-vec). 835 lines. |
...
| `okp.py` | `OKPMetadataProcessor` -- parses TOML frontmatter from OKP/errata files. Helpers: `parse_metadata()`, `yield_files_related_to_projects()`, `is_file_related_to_projects()`, `metadata_has_url_and_title()`. 152 lines. |
| `utils.py` | `get_common_arg_parser()` -- shared CLI argument definitions (folder, model-dir, chunk, overlap, output, index, workers, vector-store-type, auto-chunking). 71 lines. |
...
| `lsc/Makefile` | Build/test automation for the lsc library (`test-unit`, `verify`). |
| `lsc/pyproject.toml` | lsc library project metadata (src/ layout). |
| `lsc/uv.lock` | uv lockfile for the lsc library. |
| `lsc/rpms.in.yaml` / `lsc/rpms.lock.yaml` | RPM dependency spec + lockfile for the lsc Konflux build. |
...
| `requirements-build.in` / `requirements-build.txt` | Build-time pip dependency spec + compiled output for Cachi2. |
| `ARCHITECTURE.md` | Human-facing architecture overview (diagrams, prose). Not agent context -- agents use `.ai/spec/`. |
